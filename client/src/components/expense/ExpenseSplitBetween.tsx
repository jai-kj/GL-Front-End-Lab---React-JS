import { DropdownList, DropdownSelected } from "../../model/IDropdown"

const ExpenseSplitBetween = ({ list, id, label }: DropdownList) => {
    return (
            ...
            <div className='flex flex-col  sm:flex-row bg-dark rounded-md p-2 sm:flex-wrap'>
                {list?.map((listItem: DropdownSelected, i: number) => (
                    <div
                        ...
                        key={listItem.value}
                    >
                        <input type="checkbox" value="" placeholder="check-box" className="mr-2 w-5 h-5 cursor-pointer" />
